...
export type NewsSection = Omit<NewsItem, "type"> & {
  href: string;
...
export const NEWS_SECTIONS: readonly NewsSection[] = [
  {
    href: "/new?type=notice",
    types: ["notice"],
    label: "공지사항",
    eyebrow: "NOTICE",
    title: "런조아의 중요한 안내",
...
export function getNewsSection(type: NewsType) {
  return NEWS_SECTIONS.find((section) => section.types.includes(type))!;
}

export function getNewsSlugs(
  types: readonly NewsType[] = NEWS_ITEMS.map((item) => item.type),
) {
  return NEWS_ITEMS.filter((item) => types.includes(item.type)).flatMap(
    (item) =>
...
export async function getNewsPosts(types: NewsType | readonly NewsType[]) {
  const selectedTypes: readonly NewsType[] = Array.isArray(types)
    ? types
    : [types];
  ...
  return posts
    .filter(
      (post): post is NewsPostMetadata =>
        Boolean(post && selectedTypes.includes(post.type)),
    )
    .sort((a, b) => b.date.localeCompare(a.date));
}

export function getPostHref(post: Pick<NewsPostMetadata, "slug" | "type">) {
  const basePath =
    post.type === "blog"
      ? "/blog"
