...
import {
  ...
  getValidKakaoAccessToken,
  type KakaoConnection,
} from "@/lib/kakao/calendar";
...
export async function POST(request: Request) {
  ...
  if (connectionError || !connection) {
    return NextResponse.json(
      { error: "카카오 캘린더 연결이 필요합니다.", code: "RECONNECT_REQUIRED" },
      { status: 403 },
    );
  }
  ...
  const accessToken = await getValidKakaoAccessToken(
    admin,
    connection as KakaoConnection,
  ).catch(() => null);
  if (!accessToken) {
    return NextResponse.json(
      { error: "카카오 연결이 만료되었습니다.", code: "RECONNECT_REQUIRED" },
      { status: 403 },
    );
  }
  ...
  if (reservation.status === "pending") {
    return NextResponse.json(
      { error: "추가 요청을 확인 중입니다. 톡캘린더에서 저장 여부를 확인해 주세요.", code: "ALREADY_PROCESSING" },
      { status: 409 },
    );
  }
  ...
  const result = await createKakaoCalendarEvent(accessToken, marathon);
  if (!result.success) {
    await admin
    ...
    return NextResponse.json(
      {
        ...
        code: result.reconnectRequired ? "RECONNECT_REQUIRED" : "KAKAO_ERROR",
      },
  ...
  if (insertError) {
    ...
    return NextResponse.json({
      success: true,
      warning: "카카오에는 추가됐지만 런조아 내역 저장에 실패했습니다.",
    });
  }
