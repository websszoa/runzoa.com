...
export async function POST(request: Request) {
  ...
  if (connectionError || !connection) {
    return NextResponse.json(
      { error: "구글 캘린더 연결이 필요합니다.", code: "RECONNECT_REQUIRED" },
      { status: 403 },
    );
  }
  ...
  if (!accessToken) {
    return NextResponse.json(
      { error: "구글 연결이 만료되었습니다.", code: "RECONNECT_REQUIRED" },
      { status: 403 },
    );
  }
  ...
  const uid = createHash("sha256").update(`${user.id}:${marathon.slug}`).digest("hex");
  const result = await createGoogleCalendarEvent(accessToken, marathon, uid);
  if (!result.success) {
    await admin
    ...
    return NextResponse.json(
      {
        ...
        code: result.reconnectRequired ? "RECONNECT_REQUIRED" : "GOOGLE_ERROR",
      },
  ...
  if (insertError) {
    ...
    return NextResponse.json({
      success: true,
      warning: "구글에는 추가됐지만 런조아 내역 저장에 실패했습니다.",
    });
  }
...
async function reserveCalendarEvent(
  ...
  if (existing.data.status === "pending" && Date.parse(existing.data.updated_at) > Date.now() - 120_000) return { status: "pending" as const };
